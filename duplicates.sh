...
remove=false # remove files; otherwise, only print duplicates
keep_first=true # delete all but the first file; otherwise, delete all but the last
...
sums=$(find . $find_args -type f | sort | tr "\n" "\0" | xargs -0 md5sum | \
